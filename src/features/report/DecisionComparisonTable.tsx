...
import DecisionComparisonCards from './DecisionComparisonCards'
...
export default function DecisionComparisonTable({ initial, revised, changedPermissionIds }: DecisionComparisonTableProps): ReactElement {
  const headingId = `${useId()}-decision-comparison-title`
  ...
  return (
    <section className="decision-comparison" aria-labelledby={headingId}>
      <h4 id={headingId}>권한별 비교</h4>
        ...
        <table>
          ...
          <tbody>
            {PERMISSION_IDS.map((permissionId) => {
              ...
              return (
                <tr key={permissionId}>
                  <th scope="row"><PermissionGlyph permissionId={permissionId} /><span>{definition.label}</span><span className="permission-shape-label" aria-hidden="true">모양: {definition.shapeLabel}</span></th>
      ...
      <DecisionComparisonCards initial={initial} revised={revised} changedPermissionIds={changedPermissionIds} />
    </section>
  )
}
